  ...
  return (
    <section className={`rounded-lg border border-emerald-300/20 bg-emerald-300/[0.06] ${compact ? 'p-5' : 'p-6 md:p-7'}`}>
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
        <div>
          <p className="label text-emerald-200">{accessActive ? getPlanName(user?.activePlan) : 'Free Enterprise Pilot'}</p>
          <h2 className={`${compact ? 'mt-2 text-2xl' : 'mt-3 text-3xl md:text-4xl'} font-black text-white`}>
            Early users can use the governance workflow for free.
          </h2>
          <p className="mt-3 max-w-3xl text-sm font-semibold leading-relaxed text-zinc-400">
            Create reports, review the savings workflow, and shape the enterprise product before paid plans are turned on. No card is required.
          </p>
        </div>
        ...
        <div className="grid gap-3">
          ...
          <p className="text-center text-xs font-bold text-zinc-500">
            Payment is built in for later, but pilot users are free now.
          </p>
        </div>
      </div>
